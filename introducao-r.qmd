...
#### Como saber os argumentos de uma função??

```{r}
#| label: argumentos-função

??plot # F1 ajuda a ter ajuda direto
```

#### Outros estilos de gráfico

```{r}
#| label: exemplo-boxplot
...
### Manipulação de dados em um data.frame

Existem várias formas de manipular dados em um data.frame, vamos abordar algumas.

#### Adicionando uma nova coluna

Podemos adicionar uma nova coluna usando o operador $.

```{r}
#| label: adicionar-nova-coluna
...
#### Adicionando uma nova coluna baseado nos dados usando ifelse

A estrutura do ifelse pode ser utilizada para classificar dados de uma coluna em uma outra coluna por meio de um teste lógico.

```{r}
#| label: ifelse-data.frame-adicionar-coluna-classificacao

# Vamos adicionar uma coluna peso
df$classificacao_peso <- ifelse(df$peso < 50, "Magro",
                                ifelse(df$peso < 80, "Normal",
                                       ifelse(df$peso < 100, "Sobrepeso", "Obeso")))

df

# Também podemos operações nas colunas usando o $

df$peso_gramas <- df$peso * 1000
...
**Sempre documente seu código** com comentários explicativos, isso servirá para quando você utilizar o código novamente no futuro.
...
1.  Crie um data frame com dados de 10 pacientes, incluindo as seguintes variáveis: nome, idade, peso, altura e diagnóstico
